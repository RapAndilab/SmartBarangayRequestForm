...
if (!empty($_SESSION['birthdate'])) {
    $timestamp = strtotime($_SESSION['birthdate']);
    if ($timestamp !== false) {
        $formatted_date = date("m/d/Y", $timestamp);
    } else {
        $formatted_date = ''; // or some default/fallback
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Barangay Clearance - Barangay E-Forms</title>
...
<div class="content-container">
    ...
    <form id="docForm" method="POST">
        <input type="hidden" name="document_type" id="document_type" value="clearance">
        ...
        <input type="text" name="birth_date" value="<?= $formatted_date ?>" placeholder="MM/DD/YYYY" required>

        <label>Place of Birth:</label>
        <input type="text" name="birth_place" placeholder="Enter your place of birth" required>

        <label>Civil Status:</label>
        <input type="text" name="civil_status" placeholder="e.g. Single, Married" required>

        <label>Citizenship:</label>
        <input type="text" name="citizenship" placeholder="e.g. Filipino" required>
